...
dataloader = DataLoader(taping_dataset, batch_size=100, shuffle=True)
...
for epoch in range(epochs):
    ...
    for images, masks, prompts, _ in tqdm(dataloader, desc=f"Epoch {epoch+1}/{epochs}"):
        ...
        # Prepare inputs
        # Convert torch tensors back to PIL for processor
...
for images, masks, prompts, img_names in dataloader:
    ...
    for i in range(len(images)):
        ...
        save_path = os.path.join(
            "visualizations",
            f"{img_names[i].split('.')[0]}__{prompts[i].replace(' ', '_')}"
        )
